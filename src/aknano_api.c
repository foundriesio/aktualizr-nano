...
#warning "Compiling broken image for rollback test"
        LogInfo((AKNANO_TEST_MESSAGE_PREAMBLE "/REVISION=v%u ROLLBACK_TEST=1", aknano_settings->running_version));
...
void aknano_sample_loop(uint32_t *remaining_iterations)
{
    ...
    while (remaining_iterations == NULL || *remaining_iterations > 0) {
        ...
        if (checkin_result == 0) {
            ...
            bool is_reboot_required = false;
            if (!any_checkin_ok) {
                LogInfo((AKNANO_TEST_MESSAGE_PREAMBLE "/REVISION=v%u CHECKIN=successful", aknano_settings.running_version));
            }
